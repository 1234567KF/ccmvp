#!/usr/bin/env node
/**
 * Hook Handler (通用 IDE 手动触发版)
 * Dispatches hook events to the appropriate helper modules.
 *
 * 【通用 IDE 适配说明】
 * 原 Claude Code 版通过生命周期钩子自动触发（pre-bash/post-edit 等），
 * 本版改为手动调用：在需要时执行对应命令。
 * 通用 IDE 无自动 hook 机制，所有操作需显式触发。
 *
...
const handlers = {
  ...
  'session-restore': async () => {
    ...
    // Auto-start perf dashboard (silent background daemon)
    try {
      require('child_process').execSync(`node "${path.join(__dirname, '..', 'perf', 'perf-daemon.cjs')}" start`, { stdio: 'pipe', timeout: 5000 });
    } catch {}
  },
  ...
  'session-end': async () => {
    ...
    // Auto-stop perf dashboard
    try {
      require('child_process').execSync(`node "${path.join(__dirname, '..', 'perf', 'perf-daemon.cjs')}" stop`, { stdio: 'pipe', timeout: 5000 });
    } catch {}
  },
