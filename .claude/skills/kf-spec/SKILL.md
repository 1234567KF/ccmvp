...
A. MVP 模式（快速 Demo）
   ...
   前端：Vue 3 + Vite（UI 框架由推荐引擎+用户决策选定，见下面说明）
...
**UI 框架选择说明**：
- 若本技能由 **kf-mvp 调用**（Phase 0 → Phase 3）：UI 框架已在 Phase 0 选定，记录在 `memory/mvp-generation-log.md`。读取该文件获取选定框架（默认 Ant Design Vue）。
- 若 **独立使用**：根据项目类型从 6 框架中推荐（admin→Ant Design Vue, e-commerce→Element Plus, h5→Vant, brand→Tailwind/shadcn 等），用户确认后写入记录。
...
- **选 A（MVP 模式）**：使用 `mvp技术栈.md` 定义的技术栈作为默认值，后续 Spec 的技术方案按此生成。UI 框架优先读取 `memory/mvp-generation-log.md` 中的选定记录（由 kf-mvp Phase 0 写入），若无记录则按项目类型推荐。PRD 第 8 章「技术约束」中自动标注 Mock 策略（第三方全部 Mock，签名一致可切换），Spec 第 4 章「架构约束」中注明 Demo→生产演进路径（SQLite → MySQL，Mock → 真实服务）
