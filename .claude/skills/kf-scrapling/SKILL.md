...
name: kf-scrapling
description: |
  Web 爬虫抓取数据。基于 Scrapling（Python 库），支持反反爬绕过（Cloudflare Turnstile）、隐身浏览器、自适应元素追踪、
  Spider 并发框架、JS 渲染。触发词："爬虫"、"抓取"、"爬取"、"scrape"、"crawl"、"数据采集"、"网页抓取"、
  "web_fetch失败时重试"、"反反爬"。
  可被 kf-mvp 调研阶段自动调用，替代/补充 kf-web-search 进行深度数据采集。
triggers:
  - 爬虫
  - 抓取
  - 爬取
