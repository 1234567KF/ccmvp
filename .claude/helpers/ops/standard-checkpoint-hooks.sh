...
pre_edit_checkpoint() {
    ...
    if [ -n "$file" ]; then
        ...
        # Store metadata
        mkdir -p {IDE_ROOT}/checkpoints
        cat > "{IDE_ROOT}/checkpoints/$(date +%s).json" <<EOF
{
...
task_checkpoint() {
    ...
    if [ -n "$task" ]; then
        ...
        # Store metadata
        mkdir -p {IDE_ROOT}/checkpoints
        cat > "{IDE_ROOT}/checkpoints/task-$(date +%s).json" <<EOF
{
...
session_end_checkpoint() {
    ...
    local summary_file="{IDE_ROOT}/checkpoints/summary-$session_id.md"
...
## Checkpoints Created
$(find {IDE_ROOT}/checkpoints -name '*.json' -mtime -1 -exec basename {} \; | sort)
